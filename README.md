...
The pipeline can be applied to longitudinal datasets by storing the T1-weighted and FLAIR images from the first imaging
visit in `data/<subject>/ses-1/anat`, with images from subsequent visits stored in sequential session folders
(e.g., `ses-2`, `ses-3`) and running the pipeline as described above.

The pipeline automatically outputs a csv file
`data/derivatives/enigma-pd-wml/t1_volumes.csv` which contains the estimated brain volume (in mm³) in native T1-weighted space and MNI space, as well as the factor used to scale the brain from T1-weighted to MNI space for each subject and session.

These values are generated from `fsl_anat` and can be used for head-size normalisation in longitudinal analyses (for more information see the
 [FSL documentation](https://fsl.fmrib.ox.ac.uk/fsl/docs/structural/fsl_anat.html)).

> [!NOTE]
> The `t1_volumes.csv` file is automatically generated by the pipeline regardless of whether is it being applied to cross-sectional or longitudinal data.
