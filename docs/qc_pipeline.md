...
| Total PNGs per scan  | 728 | l182 x 2 x 2 |

### Log output

The script outputs processing status to stdout for each subject/session (saved to their individual log files at e.g.
`data/derivatives/enigma-pd-wml/sub-1/ses-1/sub-1_ses-1_results.log`). These logs are essential for identifying failed
